...
Re-runs the assessment under a strictly tighter validator. Requires status `RULED`. When
rounds are exhausted the case becomes `DEADLOCKED` instead.
...
`prose_comparison_from_round`, evidence limits.
...
`MAX_TEXT_FIELD = 2000` · `MAX_ROUNDS = 4`.
