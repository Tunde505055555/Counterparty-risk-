...
def test_deadlock_is_reachable_by_the_authorized_challengers() -> None:
    """Challenging is capped at one per case party, so the round ladder must be
    no longer than the party count or DEADLOCKED can never be reached."""
    src = CONTRACT_PATH.read_text(encoding="utf-8")
    tolerances = json.loads(
        re.search(r"SCORE_TOLERANCE_BY_ROUND = (\[[^\]]*\])", src).group(1)
    )
    max_parties = int(re.search(r"MAX_PARTIES = (\d+)", src).group(1))
    assert max_parties == 3, "requester, reviewer, owner"
    assert len(tolerances) == max_parties
    assert "all_parties_spent" in src, (
        "a case must also deadlock once every distinct party has challenged"
    )
